...
public static class ServerSubscriptions
{
	...
	{
		InvokeHandler.InvokeRepeating(Rust.Global.Runner, SendPlayerPositions, 0f, 0.2f);
		InvokeHandler.InvokeRepeating(Rust.Global.Runner, SendFoundations, 0f, 5f);
	}

	public static void StopSubscribeFeed()
	{
		InvokeHandler.CancelInvoke(Rust.Global.Runner, SendPlayerPositions);
		InvokeHandler.CancelInvoke(Rust.Global.Runner, SendFoundations);
	}

	private static void SendFoundations()
	{
		if (!Rust.Application.isServerStarted || !RCon.HasSubscribers)
		{
			return;
		}
		...
		foreach (KeyValuePair<uint, BuildingManager.Building> item in buildingDictionary)
		{
			...
			{
				if (buildingBlock.name.Contains("foundation") && !buildingBlock.name.Contains("foundation.steps"))
				{
	...
	private static void SendPlayerPositions()
	{
		...
		try
		{
			...
			textTable.ShouldPadColumns = false;
			ConVar.Server.GetPlayerListPosTable(textTable);
			RCon.SendMessageToSubscribers(textTable.ToJson(), -64001);
		}
		catch (Exception exception)
		{
			Debug.LogException(exception);
