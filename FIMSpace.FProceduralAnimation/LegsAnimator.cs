...
public class LegsAnimator : MonoBehaviour, IDropHandler, IEventSystemHandler, IFHierarchyIcon
{
	...
	public class Leg
	{
		...
		public void User_OverrideRaycastHit(Transform tr, bool disableSourceRaycast = true)
		{
			...
			_CustomRaycastOnBlendIn(new RaycastHit
			{
				point = tr.position,
				normal = tr.up
			});
	...
	protected void Hips_PreCalibrate()
	{
		...
		if (Calibrate != ECalibrateMode.None && SpineBone != null)
		{
	...
	{
		User_MoveLegTo(legIndex, new RaycastHit
		{
			point = position,
			normal = normal
		});
	...
	{
		Vector3 vector = ToRootLocalSpaceVec(worldMoveDirection);
		vector.y = 0f;
		Vector3 to = ToRootLocalSpaceVec(currentWorldLookForwardDirection);
		to.y = 0f;
		vector.Normalize();
		to.Normalize();
		return 0f - Vector3.SignedAngle(vector, to, Vector3.up);
	}
