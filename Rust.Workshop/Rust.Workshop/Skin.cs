...
public class Skin
{
	...
	public IEnumerator LoadIcon(ulong workshopId, string directory = null, AssetBundle bundle = null)
	{
		...
		if (manifestContent == null && directory != null)
		{
			...
			{
				yield return Global.Runner.StartCoroutine(ParallelEx.Coroutine(LoadManifestFromFile));
			}
		}
		if (manifestContent != null)
		{
			yield return Global.Runner.StartCoroutine(ParallelEx.Coroutine(DeserializeManifest));
		}
		if (manifest == null)
		{
		...
		if (this.sprite == null && SteamClient.IsValid)
		{
			...
			if (definitions != null)
			{
				yield return Global.Runner.StartCoroutine(ParallelEx.Coroutine(FindURL));
			}
	...
	public IEnumerator LoadAssets(ulong workshopId, string directory = null, AssetBundle bundle = null, bool compressTexturesOnLoad = true)
	{
		...
		for (int i = 0; i < Skinnable.Groups.Length && i < manifest.Groups.Length; i++)
		{
			...
			Manifest.Group obj = manifest.Groups[i];
			if (obj.Floats != null)
			{
			...
			if (obj.Colors != null)
			{
				foreach (KeyValuePair<string, Manifest.ColorEntry> color in obj.Colors)
				{
					material.SetColor(color.Key, new Color(color.Value.r, color.Value.g, color.Value.b));
				}
			}
			foreach (KeyValuePair<string, string> mat in obj.Textures)
			{
				if (mat.Value == "none")
				{
				...
				if (texture2D == null && directory != null)
				{
					...
					{
						AsyncTextureLoad request = new AsyncTextureLoad(textureName, isNormalMap, !isNormalMap, hqmode: true, cache: false, compressTexturesOnLoad);
	...
	public IEnumerable<Texture2D> GetAllTextures()
	{
		if (TextureObjects != null)
		{
			...
			{
				if (textureObject is Texture2D texture2D)
				{
		...
		{
			if (textureAsset is Texture2D texture2D2)
			{
	...
	internal void ReadDefaults()
	{
		...
		{
			Skinnable.Group obj = Skinnable.Groups[i];
			if (obj != null)
			{
				DefaultMaterials[i] = obj.Material;
			}
		}
		for (int j = 0; j < Materials.Length; j++)
		{
			...
			Material obj2 = (Materials[j] = new Material(DefaultMaterials[j]));
			obj2.DisableKeyword("_COLORIZELAYER_ON");
			obj2.SetInt("_COLORIZELAYER_ON", 0);
			obj2.name = DefaultMaterials[j].name + " (Editing)";
		}
	}
}
