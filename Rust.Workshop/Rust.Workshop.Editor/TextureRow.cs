...
public class TextureRow : MaterialRow
{
	...
	public void SetFilenameText(string filename, UnityEngine.Texture texture = null)
	{
	...
	public IEnumerator BrowseForTexture()
	{
		...
		string windowNameSuffix = ((num > 0) ? $" ( {num} x {num} ) " : "");
		yield return StartCoroutine(base.Editor.FileDialog.Open(null, ".png|.jpg", "open_texture_suffixed", windowNameSuffix));
	...
	{
		yield return StartCoroutine(base.Editor.FileDialog.Save(null, ".png", "download_file", "", saveLastPath: true, TargetImage.texture.name));
		if (!string.IsNullOrEmpty(base.Editor.FileDialog.result))
		{
			...
			bool num = IsNormalMap && texture2D.format != TextureFormat.ARGB32 && texture2D.format != TextureFormat.RGBA32;
			if (num)
			{
