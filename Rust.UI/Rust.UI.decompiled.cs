...
public class RustUiTest : MonoBehaviour
{
	...
	private void Awake()
	{
		...
		{
			UnityEngine.Debug.Log("Value Changed: " + val.Value);
		});
		...
		{
			UnityEngine.Debug.Log("Value Changed: " + val);
		});
		...
		rustButton4.OnPressed.AddListener(delegate
		{
			...
			file.Text.SetText(array[UnityEngine.Random.Range(0, array.Length)]);
		});
	...
	private static MonoScriptData Get()
	{
		return new MonoScriptData
		{
			FilePathsData = new byte[2833]
			{
				...
				103, 92, 70, 105, 108, 101, 68, 105, 97, 108,
				111, 103, 70, 97, 118, 111, 117, 114, 105, 116,
				101, 80, 97, 116, 104, 66, 117, 116, 116, 111,
				110, 46, 99, 115, 0, 0, 0, 1, 0, 0,
				0, 64, 92, 65, 115, 115, 101, 116, 115, 92,
				80, 108, 117, 103, 105, 110, 115, 92, 82, 117,
				115, 116, 46, 85, 73, 92, 84, 104, 105, 114,
				100, 80, 97, 114, 116, 121, 92, 70, 105, 108,
				101, 68, 105, 97, 108, 111, 103, 92, 70, 105,
				108, 101, 76, 105, 115, 116, 69, 108, 101, 109,
				101, 110, 116, 46, 99, 115, 0, 0, 0, 1,
				0, 0, 0, 46, 92, 65, 115, 115, 101, 116,
				115, 92, 80, 108, 117, 103, 105, 110, 115, 92,
				82, 117, 115, 116, 46, 85, 73, 92, 85, 116,
				105, 108, 105, 116, 121, 92, 73, 65, 117, 116,
				111, 76, 97, 121, 111, 117, 116, 46, 99, 115,
				0, 0, 0, 1, 0, 0, 0, 50, 92, 65,
				115, 115, 101, 116, 115, 92, 80, 108, 117, 103,
				105, 110, 115, 92, 82, 117, 115, 116, 46, 85,
				...
				101, 116, 115, 92, 80, 108, 117, 103, 105, 110,
				115, 92, 82, 117, 115, 116, 46, 85, 73, 92,
				85, 116, 105, 108, 105, 116, 121, 92, 84, 111,
				103, 103, 108, 101, 71, 97, 109, 101, 79, 98,
				106, 101, 99, 116, 46, 99, 115, 0, 0, 0,
				1, 0, 0, 0, 48, 92, 65, 115, 115, 101,
				116, 115, 92, 80, 108, 117, 103, 105, 110, 115,
				92, 82, 117, 115, 116, 46, 85, 73, 92, 85,
				116, 105, 108, 105, 116, 121, 92, 85, 73, 66,
				101, 104, 97, 118, 105, 111, 117, 114, 69, 120,
				46, 99, 115
			},
			TypesData = new byte[1796]
			{
				...
				0, 30, 124, 70, 105, 108, 101, 68, 105, 97,
				108, 111, 103, 70, 97, 118, 111, 117, 114, 105,
				116, 101, 80, 97, 116, 104, 66, 117, 116, 116,
				111, 110, 0, 0, 0, 0, 23, 82, 117, 115,
				116, 46, 85, 73, 124, 70, 105, 108, 101, 76,
				105, 115, 116, 69, 108, 101, 109, 101, 110, 116,
				0, 0, 0, 0, 12, 124, 73, 65, 117, 116,
				111, 76, 97, 121, 111, 117, 116, 0, 0, 0,
				0, 27, 85, 110, 105, 116, 121, 69, 110, 103,
				105, 110, 101, 124, 82, 101, 99, 116, 84, 114,
				97, 110, 115, 102, 111, 114, 109, 69, 120, 0,
				0, 0, 0, 32, 82, 117, 115, 116, 46, 85,
				73, 46, 85, 116, 105, 108, 105, 116, 121, 124,
				84, 111, 103, 103, 108, 101, 71, 97, 109, 101,
				79, 98, 106, 101, 99, 116, 0, 0, 0, 0,
				25, 85, 110, 105, 116, 121, 69, 110, 103, 105,
				110, 101, 124, 85, 73, 66, 101, 104, 97, 118,
				105, 111, 117, 114, 69, 120
			},
			TotalFiles = 52,
			TotalTypes = 70,
			IsEditorOnly = false
...
namespace Rust.UI
{
	...
	public class RustButtonGroup : RustControl
	{
		...
		public void SetOption(int i)
		{
			...
			i %= Options.Length;
			CurrentOption = i;
			base.transform.GetChild(i).GetComponent<RustButton>().Toggle(value: true);
		}
	...
	public class RustFlexText : RustText, IFlexNode
	{
		...
		void IFlexNode.MeasureHorizontal()
		{
			...
			{
				preferredValues.x += m_currentFontAsset.normalSpacingOffset;
			}
	...
	public struct Option
	{
		...
		{
			Icon = icon;
			Label = new Translate.Phrase(token, english);
			Value = value ?? token ?? english;
			Default = isdefault;
		}

		public Option(Translate.Phrase phrase, string value, bool isdefault = false, Icons icon = Icons.None)
		{
			Icon = icon;
			Label = phrase;
			Value = value;
	...
	public class RustText : TextMeshProUGUI, ILocalize
	{
		...
		public virtual void DoAutoSize()
		{
			if (base.font == null)
			{
				UnityEngine.Debug.LogWarning("null font: " + base.gameObject.name, base.gameObject);
				return;
			}
			if (AutoSetWidth)
			{
	...
	public class TabControl : RustControl
	{
		...
		public void AddTab(string name, Translate.Phrase buttontext = null, Icons icon = Icons.ExclamationSquare)
		{
			...
			GameObject obj = UnityEngine.Object.Instantiate(ButtonControl.gameObject);
			obj.name = name;
	...
	public class Video : RustControl
	{
		...
		{
			UnityEngine.Debug.Log("Video Error: \"" + message + "\"");
		}
	...
	public class HttpImage : MonoBehaviour
	{
		...
		private IEnumerator StartAndWaitForLoad(string url)
		{
			...
			if (downloadTask.IsCompletedSuccessfully)
			{
				...
				{
					UnityEngine.Debug.LogWarning("Failed to download image from " + url);
					texture2D = null;
				}
				else
				{
					...
					{
						UnityEngine.Debug.Log("Texture from " + url + " was too big, ignoring");
						UnityEngine.Object.DestroyImmediate(texture2D);
						texture2D = null;
					}
					else
			...
			{
				UnityEngine.Debug.LogWarning($"Failed to download image from {url}: {downloadTask.Exception}");
				texture2D = null;
			}
			if (texture2D == null)
			{
	...
	public class Properties
	{
		...
		private void AddControl(RustControl ctrl, LabelAttribute label)
		{
			if (label != null)
			{
				RustWrapper rustWrapper = Make.Form.Horizontal(label.Phrase, ctrl);
				rustWrapper.gameObject.name = ctrl.gameObject.name;
				rustWrapper.SetParent(Canvas);
			}
			else
			{
				ctrl.SetParent(Canvas);
			}
		}
		...
		private bool CreateStringControl(FieldInfo f)
		{
			...
			if (customAttribute2 != null && array.Length != 0)
			{
				...
				ctrl.gameObject.name = name;
				ctrl.OnChanged.AddListener(delegate(Option x)
				{
		...
		private bool CreateBooleanControl(FieldInfo f)
		{
			...
			ctrl.gameObject.name = name;
			ctrl.OnChanged.AddListener(delegate(bool x)
			{
				f.SetValue(Parent, x);
				MarkDirty();
			});
		...
		private bool CreateNumericControl(FieldInfo f)
		{
			...
			_ = f.FieldType == typeof(int);
			LabelAttribute customAttribute = f.GetCustomAttribute<LabelAttribute>();
			RangeAttribute customAttribute2 = f.GetCustomAttribute<RangeAttribute>();
			if (customAttribute2 != null)
			{
				RustSlider ctrl = Make.Slider(customAttribute2.min, customAttribute2.max, 0.5f);
				ctrl.gameObject.name = name;
				ctrl.OnChanged.AddListener(delegate(float x)
				{
					f.SetValue(Parent, x);
					MarkDirty();
				});
		...
		public void SetValues(Dictionary<string, object> a, Dictionary<string, object> b, float delta)
		{
			foreach (FieldInfo field in Fields)
			{
				...
				else if (field.FieldType == typeof(Vector3))
				{
					...
					{
						Quaternion quaternion = Quaternion.Lerp(Quaternion.Euler((Vector3)value), Quaternion.Euler((Vector3)value2), delta);
						field.SetValue(Parent, quaternion.eulerAngles);
					}
					else
					{
						Vector3 vector = Vector3.Lerp((Vector3)value, (Vector3)value2, delta);
						field.SetValue(Parent, vector);
					}
				}
				else
				{
		...
		{
			JObject jObject = entry.Value as JObject;
			foreach (FieldInfo field in Fields)
			{
				if (jObject.TryGetValue(field.Name, out var value))
				{
					field.SetValue(Parent, value.ToObject(field.FieldType));
				}
			}
		}
	}
