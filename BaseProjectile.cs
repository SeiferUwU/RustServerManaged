...
public class BaseProjectile : AttackEntity
{
	...
	public override void ServerUse(float damageModifier = 1f, float speedModifier = 1f, Matrix4x4? originOverride = null, bool useBulletThickness = true)
	{
		...
		float num = ammoInfo.projectileVelocity * speedModifier;
		...
		{
			Vector3 vector2 = ((!flag2) ? AimConeUtil.GetModifiedAimConeDirection(ammoInfo.projectileSpread + GetAimCone(), inputVec) : AimConeUtil.GetModifiedAimConeDirection(ammoInfo.projectileSpread + GetAimCone() + GetAIAimcone(), inputVec));
			...
			for (int j = 0; j < obj.Count && damageModifier != 0f; j++)
			{
				...
				if (entity is BasePlayer basePlayer && basePlayer.GetActiveShield(out var foundShield) && foundShield.RaycastAgainstColliders(ray, 300f))
				{
					Vector3 vector3 = base.transform.InverseTransformPoint(foundShield.transform.position);
					Vector3 vector4 = base.transform.InverseTransformPoint(basePlayer.CenterPoint());
				...
				if ((entity != null && entity.IsNpc && flag2 && baseCombatEntity != null && baseCombatEntity.GetFaction() != BaseCombatEntity.Faction.Horror && !(entity is BasePet)) || !(entity != null) || (!(baseEntity == null) && !(entity == baseEntity) && !entity.EqualNetID(baseEntity)) || !entity.IsVisible(vector, hit.point, 300f))
				{
					continue;
				}
				...
				info.DoHitEffects = component.doDefaultHitEffects;
				info.DidHit = true;
				info.ProjectileVelocity = vector2 * 300f;
				...
				info.damageTypes.ScaleAll(GetDamageScale() * damageModifier * (flag2 ? npcDamageScale : turretDamageScale));
			...
			Vector3 vector5 = ((flag && ownerPlayer.isMounted) ? (vector2 * 6f) : Vector3.zero);
			CreateProjectileEffectClientside(ammoInfo.GetOverrideProjectile(this).resourcePath, vector + vector5, vector2 * num, UnityEngine.Random.Range(1, 100), null, IsSilenced(), forceClientsideEffects: true, null, distanceOverride);
		}
	...
	public Projectile.Modifier GetProjectileModifier()
	{
		return new Projectile.Modifier
		{
			...
			damageScale = ProjectileWeaponMod.Mult(this, (ProjectileWeaponMod x) => x.projectileDamage, (ProjectileWeaponMod.Modifier y) => y.scalar, 1f) * GetDamageScale(),
			...
			distanceScale = ProjectileWeaponMod.Mult(this, (ProjectileWeaponMod x) => x.projectileDistance, (ProjectileWeaponMod.Modifier y) => y.scalar, 1f) * GetDistanceScale()
		};
	}
