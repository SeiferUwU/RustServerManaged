...
public static class HarmonyLoader
{
	...
	public static IEnumerable<HarmonyModInfo> GetHarmonyMods()
	{
		...
		return loadedMods.Select((HarmonyMod x) => new HarmonyModInfo
		{
			Name = x.Name,
			Version = (x.Assembly?.GetName()?.Version?.ToString() ?? "unknown")
		}).ToArray();
	}
	...
	public static void LoadHarmonyMods()
	{
		try
		{
			...
			UnityEngine.Debug.Log("Set managed directory to '" + directoryName + "'");
			UnityEngine.Debug.Log("Set harmony directory to '" + modPath + "'");
			AppDomain.CurrentDomain.AssemblyResolve += delegate(object sender, ResolveEventArgs args)
			{
				UnityEngine.Debug.Log("Trying to load assembly: " + args.Name);
	...
	{
		UnityEngine.Debug.Log($"[HarmonyLoader {harmonyId}] {message}");
	}

	private static void LogError(string harmonyId, object message)
	{
		UnityEngine.Debug.LogError($"[HarmonyLoader {harmonyId}] {message}");
	}

	private static void LogError(object message)
	{
		UnityEngine.Debug.LogError($"[HarmonyLoader] {message}");
	}

	private static void LogWarning(object message)
	{
		UnityEngine.Debug.LogWarning($"[HarmonyLoader] {message}");
	}
}
[CompilerGenerated]
