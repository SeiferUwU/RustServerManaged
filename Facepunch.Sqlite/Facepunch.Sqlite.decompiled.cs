...
public class Database
{
	...
	protected void DebugLogColumns(IntPtr stmHandle)
	{
		...
		UnityEngine.Debug.Log($"{num} columns");
		for (int i = 0; i < num; i++)
		{
			IntPtr ptr = Interop.sqlite3_column_name(stmHandle, i);
			UnityEngine.Debug.Log($"Column {i + 1}: {Marshal.PtrToStringAnsi(ptr)} ({Interop.GetTypeName(Interop.sqlite3_column_type(stmHandle, i))})");
		}
	}
}
