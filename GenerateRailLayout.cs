	...
	public override void Process(uint seed)
	{
		...
		foreach (MonumentInfo monument in TerrainMeta.Path.Monuments)
		{
			...
			foreach (TerrainPathConnect terrainPathConnect in array3)
			{
				...
				foreach (PathList item in list)
				{
					...
					{
						PathFinder.Point point3 = PathFinder.GetPoint(points2[num3], length);
				...
				for (int num4 = 0; (num4 < 8 && pathFinder.Heuristic(point6, list2) > 1) || (num4 < 16 && !pathFinder.IsWalkable(point6)); num4++)
				{
					list5.Add(position3);
					position3 += vector3;
				...
				if (node3 == null && list.Count > 0 && a != int.MaxValue)
				{
					...
					list4.Clear();
					for (int num5 = 0; num5 < pathList.Path.Points.Length; num5++)
					{
						list4.Add(PathFinder.GetPoint(pathList.Path.Points[num5], length));
					}
				...
				if (!flag)
				{
					foreach (PathList rail4 in TerrainMeta.Path.Rails)
					{
						...
						{
							float num9 = Vector3.Distance(a2, points3[num8]);
							if (num9 < num6)
							{
								num6 = num9;
								pathList2 = rail4;
				...
				else
				{
					foreach (PathList item2 in list)
					{
						...
						for (int num10 = 0; num10 < points4.Length; num10++)
						{
							float num11 = Vector3.Distance(a2, points4[num10]);
							if (num11 < num6)
							{
								num6 = num11;
								pathList2 = item2;
				...
				if (!flag)
				{
					Vector3 tangentByIndex = pathList2.Path.GetTangentByIndex(num7);
				...
				Vector3 vector4 = Vector3.Normalize(pathList2.Path.GetPointByIndex(num7 + b * 8 * 2) - pathList2.Path.GetPointByIndex(num7));
				...
				{
					float t = Mathf.InverseLerp(7f, 0f, num12);
					float num13 = Mathf.SmoothStep(0f, 2f, t) * 4f;
					list5.Add(pathList2.Path.GetPointByIndex(num7 + b * num12) + vector5 * num13);
				}
				if (list5.Count >= 2)
				{
					...
					{
						float a3 = Mathf.InverseLerp(0f, 8f, i);
						float b2 = Mathf.InverseLerp(rail2.Path.DefaultMaxIndex, rail2.Path.DefaultMaxIndex - 8, i);
						return Mathf.SmoothStep(0f, 1f, Mathf.Min(a3, b2));
					};
		...
		static float DistanceToRail(Vector3 vec)
		{
			...
			foreach (PathList rail5 in TerrainMeta.Path.Rails)
			{
				...
				{
					num14 = Mathf.Min(num14, (vec - vector6).Magnitude2D());
				}
			}
			return num14;
		}
