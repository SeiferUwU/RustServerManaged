...
public class PlaceCliffs : ProceduralComponent
{
	...
	public override void Process(uint seed)
	{
		...
		for (int num2 = 0; num2 < num; num2++)
		{
			...
			int num4 = (placeCliffParameters ? placeCliffParameters.CutoffSlopeInitial : CutoffSlopeInitial);
			int num5 = (placeCliffParameters ? placeCliffParameters.MinTerrainHeight : MinTerrainHeight);
			int num6 = (placeCliffParameters ? placeCliffParameters.MaxTerrainHeight : MaxTerrainHeight);
			int num7 = (placeCliffParameters ? placeCliffParameters.MinCliffHeight : MinCliffHeight);
			int num8 = (placeCliffParameters ? placeCliffParameters.MaxCliffHeight : MaxCliffHeight);
			float num9 = (placeCliffParameters ? placeCliffParameters.MinCliffScale : MinCliffScale);
			float num10 = (placeCliffParameters ? placeCliffParameters.MaxCliffScale : MaxCliffScale);
			...
			if (height < (float)num5 || height > (float)num6)
			{
				continue;
			}
			Vector3 normal = heightMap.GetNormal(normX, normZ);
			if (Vector3.Angle(Vector3.up, normal) < (float)num4)
			{
				continue;
			}
			Vector3 vector = new Vector3(x2, height, z2);
			Quaternion quaternion = QuaternionEx.LookRotationForcedUp(normal, Vector3.up);
			float num11 = Mathf.Max((num10 - num9) / (float)max_scale_attempts, min_scale_delta);
			for (float num12 = num10; num12 >= num9; num12 -= num11)
			{
				Vector3 pos = vector;
				Quaternion rot = quaternion * random.Object.transform.localRotation;
				Vector3 scale = num12 * random.Object.transform.localScale;
				random.ApplyDecorComponents(ref pos, ref rot, ref scale);
				if (random.ApplyTerrainFilters(pos, rot, scale) && random.ApplyTerrainAnchors(ref pos, rot, scale, AnchorModeInitial, ((FilterModeInitial & SpawnFilterMode.TerrainAnchorPoints) != 0) ? Filter : null) && !(pos.y < (float)num7) && !(pos.y > (float)num8) && random.ApplyTerrainChecks(pos, rot, scale, ((FilterModeInitial & SpawnFilterMode.TerrainCheckPoints) != 0) ? Filter : null) && random.ApplyWaterChecks(pos, rot, scale) && random.ApplyEnvironmentVolumeChecks(pos, rot, scale))
				{
