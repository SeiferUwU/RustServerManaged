...
public class SaveRestore : SingletonComponent<SaveRestore>
{
	...
	[CompilerGenerated]
	private sealed class _003CDoAutomatedSave_003Ed__20 : IEnumerator<object>, IEnumerator, IDisposable
	{
		...
		[DebuggerHidden]
		public _003CDoAutomatedSave_003Ed__20(int _003C_003E1__state)
		{
	...
	[IteratorStateMachine(typeof(_003CDoAutomatedSave_003Ed__20))]
	private IEnumerator DoAutomatedSave(bool AndWait = false)
	{
		Interface.CallHook("OnServerSave");
		return new _003CDoAutomatedSave_003Ed__20(0)
		{
	...
	public static bool Load(string strFilename = "", bool allowOutOfDateSaves = false)
	{
		...
		try
		{
			...
			using (FileStream fileStream = File.OpenRead(strFilename))
			{
				...
				if (binaryReader.ReadUInt32() != 274)
				{
	...
	public static void InitializeEntityLinks()
	{
		...
		for (int num = 0; num < array.Length; num++)
		{
			...
			if (stopwatch.Elapsed.TotalMilliseconds > 2000.0)
			{
				stopwatch.Reset();
				stopwatch.Start();
				DebugEx.Log("\t" + (num + 1) + " / " + array.Length);
			}
		}
		DebugEx.Log("\tdone.");
	}
