...
[assembly: AssemblyInformationalVersion("1.0.0+23de139dc39be14a3ff8b2508780ab8a299cb465")]
...
namespace Steamworks
{
	...
	internal class ISteamNetworkingSockets : SteamInterface
	{
		...
		private static extern Steamworks.Data.Socket _CreateListenSocketIP(IntPtr self, ref NetAddress localAddress, int nOptions, [In][Out] NetKeyValue[] pOptions);

		internal Steamworks.Data.Socket CreateListenSocketIP(ref NetAddress localAddress, int nOptions, [In][Out] NetKeyValue[] pOptions)
		{
		...
		private static extern Steamworks.Data.Socket _CreateListenSocketP2P(IntPtr self, int nLocalVirtualPort, int nOptions, [In][Out] NetKeyValue[] pOptions);

		internal Steamworks.Data.Socket CreateListenSocketP2P(int nLocalVirtualPort, int nOptions, [In][Out] NetKeyValue[] pOptions)
		{
		...
		[return: MarshalAs(UnmanagedType.I1)]
		private static extern bool _CloseListenSocket(IntPtr self, Steamworks.Data.Socket hSocket);
		...
		[return: MarshalAs(UnmanagedType.I1)]
		private static extern bool _GetListenSocketAddress(IntPtr self, Steamworks.Data.Socket hSocket, ref NetAddress address);

		internal bool GetListenSocketAddress(Steamworks.Data.Socket hSocket, ref NetAddress address)
		{
		...
		private static extern Steamworks.Data.Socket _CreateHostedDedicatedServerListenSocket(IntPtr self, int nLocalVirtualPort, int nOptions, [In][Out] NetKeyValue[] pOptions);

		internal Steamworks.Data.Socket CreateHostedDedicatedServerListenSocket(int nLocalVirtualPort, int nOptions, [In][Out] NetKeyValue[] pOptions)
		{
		...
		private static extern Steamworks.Data.Socket _CreateListenSocketP2PFakeIP(IntPtr self, int idxFakePort, int nOptions, [In][Out] NetKeyValue[] pOptions);

		internal Steamworks.Data.Socket CreateListenSocketP2PFakeIP(int idxFakePort, int nOptions, [In][Out] NetKeyValue[] pOptions)
		{
	...
	public class SteamApps : SteamSharedClass<SteamApps>
	{
		...
		public static async Task<FileDetails?> GetFileDetailsAsync(string filename)
		{
			...
			{
				SizeInBytes = fileDetailsResult_t.Value.FileSize,
				Flags = fileDetailsResult_t.Value.Flags,
				Sha1 = string.Join("", fileDetailsResult_t.Value.FileSHA.Select((byte x) => x.ToString("x")))
			};
		}
	...
	public class SteamFriends : SteamClientClass<SteamFriends>
	{
		...
		private static void OnFriendChatMessage(GameConnectedFriendChatMsg_t data)
		{
			...
			if (Internal.GetFriendMessage(data.SteamIDUser, data.MessageID, m, 32768, ref peChatEntryType) != 0 || peChatEntryType != ChatEntryType.Invalid)
			{
		...
		private static void OnGameConnectedClanChatMessage(GameConnectedClanChatMsg_t data)
		{
			...
			if (Internal.GetClanChatMessage(data.SteamIDClanChat, data.MessageID, m, 32768, ref peChatEntryType, ref psteamidChatter) != 0 || peChatEntryType != ChatEntryType.Invalid)
			{
				string arg2 = peChatEntryType.ToString();
				string arg3 = Helpers.MemoryToString(m);
		...
		public static async Task<SteamId[]> GetFollowingList()
		{
			...
			do
			{
				FriendsEnumerateFollowingList_t? friendsEnumerateFollowingList_t = (friendsEnumerateFollowingList_t2 = await Internal.EnumerateFollowingList((uint)resultCount));
				if (!friendsEnumerateFollowingList_t.HasValue)
				{
					continue;
				}
				resultCount += friendsEnumerateFollowingList_t2.Value.ResultsReturned;
				Array.ForEach(friendsEnumerateFollowingList_t2.Value.GSteamID, delegate(ulong id)
				{
					if (id != 0)
					{
						steamIds.Add(id);
			...
			while (friendsEnumerateFollowingList_t2.HasValue && resultCount < friendsEnumerateFollowingList_t2.Value.TotalResultCount);
	...
	public class SteamNetworkingSockets : SteamSharedClass<SteamNetworkingSockets>
	{
		...
		{
			NetKeyValue[] array = Array.Empty<NetKeyValue>();
			Steamworks.Data.Socket socket = Internal.CreateListenSocketIP(ref address, array.Length, array);
		...
		{
			NetKeyValue[] array = Array.Empty<NetKeyValue>();
			Steamworks.Data.Socket socket = Internal.CreateListenSocketP2P(virtualport, array.Length, array);
			SocketManager socketManager = new SocketManager
			{
				Socket = socket,
				Interface = intrface
		...
		{
			NetKeyValue[] array = Array.Empty<NetKeyValue>();
			Steamworks.Data.Socket socket = Internal.CreateListenSocketP2PFakeIP(0, array.Length, array);
			SocketManager socketManager = new SocketManager
			{
				Socket = socket,
				Interface = intrface
	...
	public class SteamServer : SteamServerClass<SteamServer>
	{
		...
		public static void Init(AppId appid, SteamServerInit init, bool asyncCallbacks = true)
		{
			...
			if (SteamInternal.GameServer_Init(num, init.GamePort, init.QueryPort, num2, init.VersionString, pszInternalCheckInterfaceVersions, out var pOutErrMsg) != SteamAPIInitResult.OK)
			{
	...
	public class SteamTimeline : SteamClientClass<SteamTimeline>
	{
		...
		public static async Task<GamePhaseRecordingInfo?> DoesGamePhaseRecordingExist(string phaseId)
		{
			...
			{
				PhaseId = value.PhaseIDUTF8(),
				RecordingMs = value.RecordingMS,
				LongestClipMs = value.LongestClipMS,
...
namespace Steamworks.Ugc
{
	...
	public struct Query
	{
		...
		public async Task<ResultPage?> GetPageAsync(int page)
		{
			...
			{
				Handle = steamUGCQueryCompleted_t.Value.Handle,
				ResultCount = (int)steamUGCQueryCompleted_t.Value.NumResultsReturned,
				TotalCount = (int)steamUGCQueryCompleted_t.Value.TotalMatchingResults,
				CachedData = steamUGCQueryCompleted_t.Value.CachedData,
