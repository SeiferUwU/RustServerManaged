...
public class TerrainHeightMap : TerrainMap<short>
{
	public struct HeightMapQueryStructure
	{
		...
		public float GetHeightFast(Vector2 uv)
		{
			...
			float num12 = (float)Data[num10] * 3.051944E-05f;
			float num13 = (float)Data[index] * 3.051944E-05f;
			float num14 = (float)Data[num11] * 3.051944E-05f;
			float num15 = (float)Data[index2] * 3.051944E-05f;
			float num16 = (num13 - num12) * num6 + num12;
			float num17 = ((num15 - num14) * num6 + num14 - num16) * num7 + num16;
			return TerrainPosition.y + num17 * TerrainSize.y;
		}
	}
	...
	public JobHandle GetHeights(NativeArray<Vector3>.ReadOnly worldPos, NativeArray<float> results)
	{
		return new GetHeightsJob
		{
			Heights = results,
			Pos = worldPos,
			Data = src.AsReadOnly(),
	...
	{
		GetHeightsJobIndirect jobData = new GetHeightsJobIndirect
		{
			Heights = results,
			Pos = worldPos,
			Data = src.AsReadOnly(),
	...
	public HeightMapQueryStructure GetQueryStructure()
	{
		return new HeightMapQueryStructure
		{
			Data = src.AsReadOnly(),
			Res = res,
			TerrainPosition = TerrainMeta.Position,
			TerrainSize = TerrainMeta.Size,
			TerrainOneOverSize = TerrainMeta.OneOverSize
		};
	}
	...
	public void GetHeightsFastIndirect(NativeArray<Vector2>.ReadOnly uvs, NativeArray<int>.ReadOnly indices, NativeArray<float> results)
	{
		GetHeightsFastJobIndirect jobData = new GetHeightsFastJobIndirect
		{
			Heights = results,
			UVs = uvs,
			Indices = indices,
			Data = src.AsReadOnly(),
			Res = res,
			TerrainPos = TerrainMeta.Position.y,
			TerrainScale = TerrainMeta.Size.y
		};
		IJobExtensions.RunByRef(ref jobData);
	}
