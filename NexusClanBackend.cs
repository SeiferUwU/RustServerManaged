...
public class NexusClanBackend : IClanBackend, IDisposable
{
	...
	{
		ClanCreateParameters parameters = new ClanCreateParameters
		{
			ClanName = name,
			ClanNameNormalized = name.ToLowerInvariant().Normalize(NormalizationForm.FormKC),
			LeaderPlayerId = NexusClanUtil.GetPlayerId(leaderSteamId),
			LeaderRoleName = "Leader",
			LeaderRoleVariables = NexusClanUtil.DefaultLeaderVariables,
	...
	public async ValueTask<ClanValueResult<List<ClanInvitation>>> ListInvitations(ulong steamId)
	{
		...
		if (nexusClanResult.IsSuccess && nexusClanResult.TryGetResponse(out var response))
		{
			List<ClanInvitation> value = response.Select((Facepunch.Nexus.Models.ClanInvitation i) => new ClanInvitation
			{
				ClanId = i.ClanId,
				Recruiter = NexusClanUtil.GetSteamId(i.RecruiterPlayerId),
	...
	public async ValueTask<ClanValueResult<List<ClanLeaderboardEntry>>> GetLeaderboard(int limit = 100)
	{
		...
		if (nexusClanResult.IsSuccess && nexusClanResult.TryGetResponse(out var response))
		{
			List<ClanLeaderboardEntry> value = response.Select((Facepunch.Nexus.Models.ClanLeaderboardEntry c) => new ClanLeaderboardEntry
			{
				ClanId = c.ClanId,
				Name = c.Name,
				Score = c.Score
