...
internal sealed class DMDCecilGenerator : DMDGenerator<DMDCecilGenerator>
{
	protected override MethodInfo GenerateCore(DynamicMethodDefinition dmd, object? context)
	{
		...
		try
		{
			...
			Relinker relinker = delegate(IMetadataTokenProvider metadataTokenProvider, IGenericParameterProvider? ctx)
			{
				if (metadataTokenProvider == def)
				{
					return clone;
				}
				return (metadataTokenProvider is MethodReference methodReference && methodReference.FullName == def.FullName && methodReference.DeclaringType.FullName == def.DeclaringType.FullName && methodReference.DeclaringType.Scope.Name == def.DeclaringType.Scope.Name) ? clone : module.ImportReference(metadataTokenProvider);
			};
