...
internal sealed class DebugLog
{
	...
	private void TryInitializeLogToFile(string file, string[]? sourceFilter, LogLevelFilter filter)
	{
		try
		{
			...
			SubscribeCore(filter, delegate(string source, DateTime time, LogLevel level2, string msg)
			{
				...
				string value3 = $"[{source}]({value2}) {level2.FastToString()}: {msg}";
				lock (sync)
				{
		...
		catch (Exception value)
		{
			...
			Instance.LogCore("DebugLog", level, ref message);
		}
	}
	...
	private void TryInitializeMemoryLog(LogLevelFilter filter)
	{
		try
		{
			...
			object sync = new object();
			_ = Encoding.UTF8;
			SubscribeCore(filter, delegate(string source, DateTime time, LogLevel logLevel2, string msg)
			{
				byte value2 = (byte)logLevel2;
				...
				lock (sync)
				{
					...
					int num3 = 0;
					System.Runtime.CompilerServices.Unsafe.WriteUnaligned(ref System.Runtime.CompilerServices.Unsafe.Add(ref reference, num3), value2);
					num3++;
