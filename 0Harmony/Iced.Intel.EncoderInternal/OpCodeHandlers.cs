...
internal static class OpCodeHandlers
{
	...
	static OpCodeHandlers()
	{
		...
		for (; i < encFlags.Length; i++)
		{
			...
			{
				Code code = (Code)i;
				opCodeHandler = ((code != Code.INVALID) ? ((code > Code.DeclareQword) ? ((code != Code.Zero_bytes) ? ((OpCodeHandler)new LegacyHandler((EncFlags1)encFlags[i], (EncFlags2)encFlags2[i], encFlags4)) : ((OpCodeHandler)new ZeroBytesHandler(code))) : new DeclareDataHandler(code)) : invalidHandler);
				break;
			}
