...
internal readonly struct ReadOnlyMemory<T>
{
	...
	public ReadOnlySpan<T> Span
	{
		...
		get
		{
			...
			{
				result = new ReadOnlySpan<T>(System.Runtime.CompilerServices.Unsafe.As<System.Pinnable<T>>(text), MemoryExtensions.StringAdjustment, text.Length);
	...
	public unsafe MemoryHandle Pin()
	{
		...
		if (typeof(T) == typeof(char) && _object is string value)
		{
			...
			return new MemoryHandle(System.Runtime.CompilerServices.Unsafe.Add<T>((void*)handle.AddrOfPinnedObject(), _index), handle);
		}
		if (_object is T[] array)
		{
			if (_length < 0)
			{
				return new MemoryHandle(System.Runtime.CompilerServices.Unsafe.Add<T>(System.Runtime.CompilerServices.Unsafe.AsPointer(ref MemoryMarshal.GetReference<T>(array)), _index));
			}
			GCHandle handle2 = GCHandle.Alloc(array, GCHandleType.Pinned);
			return new MemoryHandle(System.Runtime.CompilerServices.Unsafe.Add<T>((void*)handle2.AddrOfPinnedObject(), _index), handle2);
		}
