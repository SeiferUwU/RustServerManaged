...
internal sealed class Encoder
{
	...
	public bool TryEncode(in Instruction instruction, ulong rip, out uint encodedLength, [_003Cb2ffb5d6_002D6a81_002D4f20_002D8e75_002D7064682f7f7c_003ENotNullWhen(false)] out string? errorMessage)
	{
		...
		if (!opCodeHandler.IsSpecialInstr)
		{
			...
			if ((EncoderFlags & (EncoderFlags.ModRM | EncoderFlags.Displ)) != EncoderFlags.None)
			{
				WriteModRM();
			}
	...
	internal void AddRegOrMem(in Instruction instruction, int operand, Register regLo, Register regHi, Register vsibIndexRegLo, Register vsibIndexRegHi, bool allowMemOp, bool allowRegOp)
	{
		...
		case OpKind.Memory:
		{
			...
			if ((EncoderFlags & EncoderFlags.RegIsMemory) != EncoderFlags.None && GetRegisterOpSize(in instruction) != num)
			{
	...
	private void AddMemOp(in Instruction instruction, int operand, int addrSize, Register vsibIndexRegLo, Register vsibIndexRegHi)
	{
		...
		if ((memoryBase != Register.None && memoryBase != Register.RIP && memoryBase != Register.EIP && !Verify(operand, memoryBase, register, register2)) || (memoryIndex != Register.None && !Verify(operand, memoryIndex, register3, regHi)))
		{
			return;
		}
		...
		if (memoryBase == Register.RIP || memoryBase == Register.EIP)
		{
			...
			if ((EncoderFlags & EncoderFlags.MustUseSib) != EncoderFlags.None)
			{
		...
		if (memoryBase == Register.None && memoryIndex == Register.None)
		{
			...
			else if (bitness == 64 || internalMemoryIndexScale != 0 || (EncoderFlags & EncoderFlags.MustUseSib) != EncoderFlags.None)
			{
				ModRM |= 4;
				DisplSize = DisplSize.Size4;
	...
	internal void WritePrefixes(in Instruction instruction, bool canWriteF3 = true)
	{
		...
		if ((EncoderFlags & EncoderFlags.PF0) != EncoderFlags.None || instruction.HasLockPrefix)
		{
			WriteByteInternal(240u);
		}
		if ((EncoderFlags & EncoderFlags.P66) != EncoderFlags.None)
		{
			WriteByteInternal(102u);
		}
		if ((EncoderFlags & EncoderFlags.P67) != EncoderFlags.None)
		{
	...
	{
		if ((EncoderFlags & EncoderFlags.ModRM) != EncoderFlags.None)
		{
			WriteByteInternal(ModRM);
			if ((EncoderFlags & EncoderFlags.Sib) != EncoderFlags.None)
			{
