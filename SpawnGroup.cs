...
public class SpawnGroup : BaseMonoBehaviour, IServerComponent, ISpawnPointUser, ISpawnGroup
{
	...
	protected virtual void Spawn(int numToSpawn)
	{
		...
		for (int i = 0; i < numToSpawn; i++)
		{
			...
			if ((bool)baseEntity2)
			{
				...
				PoolableEx.AwakeFromInstantiate(baseEntity2.gameObject);
				baseEntity2.Spawn();
				PostSpawnProcess(baseEntity2, spawnPoint);
				SpawnPointInstance spawnPointInstance = baseEntity2.gameObject.AddComponent<SpawnPointInstance>();
	...
	protected virtual BaseSpawnPoint GetSpawnPoint(GameObjectRef prefabRef, out Vector3 pos, out Quaternion rot)
	{
		...
		for (int i = 0; i < spawnPoints.Length; i++)
		{
			...
			if (!(baseSpawnPoint == null) && (baseSpawnPoint.IsAvailableTo(prefabRef.Get()) || AllowOverlappingSpawns) && !baseSpawnPoint.HasPlayersIntersecting())
			{
				...
				if (NavMesh.SamplePosition(pos, out var hit, 2f, -1))
				{
					pos = hit.position;
	...
	private static bool DoesRequireNavmeshToSpawn(GameObject prefab)
	{
		...
		if (BaseNetworkableEx.Is<BaseNavigator>(prefab.GetComponent<BaseNavigator>(), out var entAsT) && entAsT.CanUseNavMesh)
		{
			return true;
		}
		if (prefab.GetComponent<LimitedTurnNavAgent>() != null)
		{
			return true;
		}
		return false;
