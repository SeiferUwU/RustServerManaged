...
public class OceanSimulation : IDisposable
{
	...
	public JobHandle TraceBatch(NativeList<Ray> rays, NativeArray<float> maxDists, NativeArray<bool> hitResults, NativeArray<Vector3> hitPositions, JobHandle inputDeps)
	{
		...
		if (num <= 0.1f)
		{
			inputDeps = new OceanSimulationJobs.SmallDisplacementPlaneTraceJob
			{
			...
			}.Schedule(rays, 256, inputDeps);
			return inputDeps;
		}
		inputDeps = new OceanSimulationJobs.OceanTraceJob
		{
			MaxDisplacement = num,
			Rays = rays,
			MaxDists = maxDists.AsReadOnly(),
			...
			HeightMapQueryStructure = TerrainMeta.HeightMap.GetQueryStructure(),
