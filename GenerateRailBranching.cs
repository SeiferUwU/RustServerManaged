...
public class GenerateRailBranching : ProceduralComponent
{
	...
	public override void Process(uint seed)
	{
		...
		foreach (PathList rail4 in TerrainMeta.Path.Rails)
		{
			...
			int num4 = path.MinIndex + 1 + 8;
			int num5 = path.MaxIndex - 1 - 8;
			for (int num6 = num4; num6 <= num5; num6++)
			{
				list2.Clear();
				list3.Clear();
				list4.Clear();
				int num7 = SeedRandom.Range(ref seed, min2, max2);
				int num8 = SeedRandom.Range(ref seed, min, max);
				int num9 = num6;
				int num10 = num6 + num7;
				if (num10 >= num5)
				{
				...
				Vector3 vector3 = tangents[num9];
				Vector3 to = tangents[num10];
				if (Vector3.Angle(vector3, to) > 30f)
				{
					continue;
				}
				Vector3 vector4 = points3[num9];
				Vector3 vector5 = points3[num10];
				if (hashSet.Contains(vector4) || hashSet.Contains(vector5))
				{
					continue;
				}
				PathFinder.Point point = PathFinder.GetPoint(vector4, length);
				PathFinder.Point point2 = PathFinder.GetPoint(vector5, length);
				num6 += num8;
				...
				int num14 = 8;
				int num15 = list4.Count - 1 - 8;
				Vector3 to2 = Vector3.Normalize(list4[num14 + 16] - list4[num14]);
				Vector3 to3 = Vector3.Normalize(list4[num15] - list4[num15 - 16]);
				Vector3 vector6 = Vector3.Normalize(points3[num9 + 16] - points3[num9]);
				Vector3 vector7 = Vector3.Normalize(points3[num10] - points3[(num10 - 16 + points3.Length) % points3.Length]);
				float num16 = Vector3.SignedAngle(vector6, to2, Vector3.up);
				float num17 = 0f - Vector3.SignedAngle(vector7, to3, Vector3.up);
				if (Mathf.Sign(num16) != Mathf.Sign(num17) || Mathf.Abs(num16) > 60f || Mathf.Abs(num17) > 60f)
				{
					continue;
				}
				Vector3 vector8 = rot90 * vector6;
				Vector3 vector9 = rot90 * vector7;
				if (num16 < 0f)
				{
					vector8 = -vector8;
				}
				...
				{
					int index = num18;
					int index2 = list4.Count - num18 - 1;
					float t = Mathf.InverseLerp(0f, 8f, num18);
					float num19 = Mathf.SmoothStep(0f, 2f, t) * 4f;
					list4[index] += vector8 * num19;
					list4[index2] += vector9 * num19;
				}
				bool flag = false;
				bool flag2 = false;
