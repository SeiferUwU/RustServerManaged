	...
	public class Arg
	{
		...
		internal bool HasPermission()
		{
			...
			if (IsClientside)
			{
				...
				{
					UnityEngine.Debug.Log("Server tried to run command \"" + FullString + "\", but we blocked it.");
					return false;
				}
				return cmd.Client;
			}
		...
		private void DefaultCall(Arg arg)
		{
			if (SetOveride != null && arg.HasArgs())
			{
				if (arg.IsClientside && Replicated)
				{
					...
					arg.Silent = true;
					UnityEngine.Debug.LogWarning("ConVar '" + Name + "' will be replicated to all other players on the server");
				}
				else
				{
					Set(arg.Args[0]);
		...
		public static void Initialize(Command[] Commands)
		{
			...
			foreach (Command command2 in array)
			{
				if (command2.Server)
				{
					...
					{
						UnityEngine.Debug.LogWarning("Server Vars have multiple entries for " + command2.FullName);
					}
					else
					{
					...
					if (command2.Replicated)
					{
						...
						{
							UnityEngine.Debug.LogWarning("Replicated server var " + command2.FullName + " has a bad config");
						}
						else
						{
				...
				if (command2.Client)
				{
					...
					{
						UnityEngine.Debug.LogWarning("Client Vars have multiple entries for " + command2.FullName);
					}
					else
					{
		...
		public static void Reset()
		{
			...
			foreach (Command command in all)
			{
				if (command.Variable && command.Default != null)
				{
					...
					{
						UnityEngine.Debug.LogError($"Exception running {command.FullName} = {command.Default}: {arg}");
					}
				}
			}
		}
...
public class Commands : ConsoleSystem
{
	...
	{
		UnityEngine.Debug.Log(fullString);
	}
}
[CompilerGenerated]
