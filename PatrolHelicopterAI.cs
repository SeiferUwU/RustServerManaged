	...
	private static MonumentInfo GetCloseMonument(float maxDistance, Vector3 position)
	{
		...
		if (TerrainMeta.Path != null && TerrainMeta.Path.Monuments != null && TerrainMeta.Path.Monuments.Count > 0)
		{
			...
			foreach (MonumentInfo monument in TerrainMeta.Path.Monuments)
			{
				...
				if (type != MonumentType.Mountain && type != MonumentType.Lighthouse && type != MonumentType.Lake && type != MonumentType.WaterWell && type != MonumentType.Cave && type != MonumentType.Building && monument.Tier != 0 && !(monument.transform.position.y < WaterSystem.OceanLevel) && monument.AllowPatrolHeliCrash)
				{
