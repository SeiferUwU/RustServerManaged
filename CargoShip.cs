...
public class CargoShip : BaseEntity
{
	...
	public Translate.Phrase monumentNotificationPhrase;
	...
	public void TriggeredEventPostSpawn()
	{
		if (ConVar.Global.legacymonumentnotifications)
		{
			ClientRPC(RpcTarget.NetworkGroup("Client_DoLegacyMonumentNotification"));
		}
		else
		{
			ClientRPC(RpcTarget.NetworkGroup("Client_DoMonumentNotification"));
		}
	}
	...
	public void SpawnSubEntities()
	{
		...
		cameraEntities = Facepunch.Pool.Get<List<CCTV_RC>>();
		CargoshipCamera[] array2 = cctvCameras;
		for (int i = 0; i < array2.Length; i++)
		{
			CargoshipCamera cargoshipCamera = array2[i];
			CCTV_RC cCTV_RC = GameManager.server.CreateEntity(cctvCameraPrefab.resourcePath, cargoshipCamera.cameraPoint.position, cargoshipCamera.cameraRotation) as CCTV_RC;
			if ((bool)cCTV_RC)
			{
	...
	{
		Facepunch.Pool.FreeUnmanaged(ref cameraEntities);
	...
	{
		base.Save(info);
		info.msg.cargoShip = Facepunch.Pool.Get<ProtoBuf.CargoShip>();
		...
		info.msg.cargoShip.playerIds = Facepunch.Pool.Get<List<ulong>>();
	...
	private void UpdateShip(float desiredThrottle, Vector3 desiredWaypoint, Vector3 approachRotationNode)
	{
		...
		turnScale = Mathf.Lerp(turnScale, num3, UnityEngine.Time.deltaTime * 0.2f);
		float num4 = ((!(num < 0f)) ? 1 : (-1));
		currentTurnSpeed = num2 * turnScale * num4;
		if (!isDoingHarborApproach)
		{
			base.transform.Rotate(Vector3.up, UnityEngine.Time.deltaTime * currentTurnSpeed, Space.World);
		}
		currentThrottle = Mathf.Lerp(currentThrottle, desiredThrottle, UnityEngine.Time.deltaTime * 0.2f);
		...
		if (isDoingHarborApproach)
		{
			...
			{
				base.transform.rotation = Quaternion.Slerp(base.transform.rotation, Quaternion.LookRotation(normalized2), UnityEngine.Time.deltaTime * 0.1f);
			}
		}
		...
		base.transform.position += currentVelocity * UnityEngine.Time.deltaTime;
	}
