...
public class TriggerParent : TriggerBase, IServerComponent
{
	...
	public virtual bool ShouldParent(BaseEntity ent, bool bypassOtherTriggerCheck = false)
	{
		...
		if (!bypassOtherTriggerCheck && !overrideOtherTriggers)
		{
			...
			if (parentEntity.IsValid() && parentEntity != GameObjectEx.ToBaseEntity(base.gameObject))
			{
				return false;
			}
		}
		if (ent.FindTrigger<TriggerParentExclusion>() != null && (!ent.FindTrigger<TriggerParentExclusion>().IgnoreIfOnLadder || !(ent is BasePlayer basePlayer) || !basePlayer.OnLadder()))
		{
			return false;
		}
	...
	protected void Unparent(BaseEntity ent)
	{
		...
		if (ent.IsValid() && !ent.IsDestroyed)
		{
			...
			if (triggerParent != null && GameObjectEx.ToBaseEntity(triggerParent.gameObject).IsValid())
			{
		...
		basePlayer.unparentTime = UnityEngine.Time.time;
	...
	private bool HasObjUnderFeet(BaseEntity ent)
	{
		...
		{
			BaseEntity toFind = GameObjectEx.ToBaseEntity(base.gameObject);
