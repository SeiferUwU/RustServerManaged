...
public class RelationshipManager : BaseEntity
{
	...
	public override void InitShared()
	{
		...
		if (base.isServer)
		{
			try
			{
				...
				Database.Open($"{ConVar.Server.rootFolder}/relationship.{274}.db");
				Database.Initialize();
			}
			catch (Exception exception)
			{
	...
	public static void sendinvite(ConsoleSystem.Arg arg)
	{
		...
		{
			float num = 7f;
			if (!(Vector3.Distance(basePlayer2.transform.position, basePlayer.transform.position) > num) && Interface.CallHook("IOnTeamInvite", basePlayer, basePlayer2) == null && Interface.CallHook("OnTeamMemberInvite", playerTeam, basePlayer, basePlayer2.userID.Get(), false) == null)
			{
