...
public class NPCTalking : NPCShopKeeper, IConversationProvider
{
	...
	public BaseMission GetGenericMissionById(uint id)
	{
		...
		for (int i = 0; i < missionList.Length; i++)
		{
			...
			if (baseMission != null && baseMission.id == id && baseMission.genericMissionProvider.isValid && baseMission.genericMissionProvider.Get().TryGetComponent<NPCSimpleMissionProvider>(out var component) && component.prefabID == prefabID)
			{
				return baseMission;
			}
		}
