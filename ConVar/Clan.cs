...
public class Clan : ConsoleSystem
{
	...
	public static void Info(Arg arg)
	{
		...
		static async void SendClanInfoConsole(long id)
		{
			try
			{
				IClan clan = await GetClanByID(id);
				if (clan != null)
				{
					Debug.Log(FormatClan(clan));
				}
			}
		...
		async void SendClanInfoPlayer(BasePlayer player)
		{
			...
			try
			{
				IClan clan = ((clanId != 0L) ? (await GetClanByID(clanId)) : (await GetPlayerClan(player)));
				IClan clan2 = clan;
				if (clan2 != null)
				{
					string msg = FormatClan(clan2);
