...
public class VendingMachine : ContainerIOEntity, IUGCBrowserEntity, IFoodSpoilModifier, PlayerInventory.ICanMoveFrom
{
	...
	private void UpdateDronePrediction(bool checkForUpdate = true)
	{
		if (predictionConfig == null)
		{
	...
	public bool DoTransaction(BasePlayer buyer, int sellOrderId, int numberOfTransactions = 1, ItemContainer targetContainer = null, Action<BasePlayer, Item> onCurrencyRemoved = null, Action<BasePlayer, Item> onItemPurchased = null, MarketTerminal droneMarketTerminal = null)
	{
		...
		{
			buyer.inventory.FindItemsByItemID(pooledList2, sellOrder.currencyIsBP ? blueprintBaseDef.itemid : sellOrder.currencyID);
			for (int num4 = 0; num4 < pooledList2.Count; num4++)
			{
				Item item = pooledList2[num4];
		...
		int num5 = GetTotalPriceForOrder(sellOrder) * numberOfTransactions;
		if (num3 < num5)
		{
			Facepunch.Pool.FreeUnmanaged(ref obj2);
			return false;
		...
		int num6 = 0;
		for (int num7 = 0; num7 < pooledList.Count; num7++)
		{
			Item item2 = pooledList[num7];
		...
		Facepunch.Rust.Analytics.Azure.OnBuyFromVendingMachine(buyer, this, sellOrder.itemToSellID, num, sellOrder.itemToSellIsBP, sellOrder.currencyID, num6, sellOrder.currencyIsBP, numberOfTransactions, sellOrder.priceMultiplier, droneMarketTerminal);
		if (ShouldRecordStats)
		{
			AddPurchaseHistory(sellOrder.itemToSellID, num, sellOrder.currencyID, num6, sellOrder.itemToSellIsBP, sellOrder.currencyIsBP);
		}
		Facepunch.Pool.FreeUnmanaged(ref obj2);
		UpdateEmptyFlag();
