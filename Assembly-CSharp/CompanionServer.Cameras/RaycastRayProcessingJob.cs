...
public struct RaycastRayProcessingJob : IJobParallelFor
{
	...
	{
		ref readonly RaycastHit reference = ref BurstUtil.GetReadonly(in raycastHits, index);
		...
		float num4 = (num ? reference.distance : farPlane);
		if (b == 7)
		{
			b = 0;
			num4 *= 1.1f;
		...
		float num6 = math.max(math.dot(cameraForward, reference.normal), 0f);
