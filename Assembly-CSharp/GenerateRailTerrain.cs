...
public class GenerateRailTerrain : ProceduralComponent
{
	...
	public override void Process(uint seed)
	{
		...
		for (int i = 0; i < 8; i++)
		{
			foreach (PathList rail in TerrainMeta.Path.Rails.AsEnumerable().Reverse())
			{
				...
				for (int j = 0; j < points.Length; j++)
				{
					...
					if (rail.Start)
					{
						vector.y = Mathf.SmoothStep(vector.y, num, SmoothenFilter(rail, j));
					}
					else
					{
						vector.y = num;
			...
			foreach (PathList item in TerrainMeta.Path.Rails.AsEnumerable().Reverse())
			{
				...
				float fademax = Mathf.InverseLerp(8f, 0f, i);
		...
		foreach (PathList rail2 in TerrainMeta.Path.Rails)
		{
			...
			{
				Vector3 vector2 = points2[num2];
				float height = heightMap.GetHeight(vector2);
				if (rail2.Start)
				{
					vector2.y = Mathf.SmoothStep(vector2.y, height, SmoothenFilter(rail2, num2));
				}
				else
				{
					vector2.y = height;
				}
				points2[num2] = vector2;
			}
