...
public class Inventory : ConsoleSystem
{
	...
	public static void setwornarmorslots(Arg arg)
	{
		...
		foreach (Item item in basePlayer.inventory.containerWear.itemList)
		{
			...
			if (component != null)
			{
				component.SetSlotAmount(item, Mathf.Min(a, component.MaxSlots));
			}
		}
	}
	...
	public static void deployLoadoutInRange(Arg arg)
	{
		...
		global::Vis.Entities(basePlayer.transform.position, radius, obj, 131072);
		int num = 0;
	...
	public static void equipslot(Arg arg)
	{
		...
		if (!(basePlayer2 == null))
		{
			...
			Debug.Log($"Equipped slot {num} on player {basePlayer2.displayName}");
		}
	}
