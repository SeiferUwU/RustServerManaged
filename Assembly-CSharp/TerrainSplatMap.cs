	...
	public void GenerateTextures()
	{
		...
		Parallel.For(0, res, delegate(int z)
		{
			for (int i = 0; i < res; i++)
			{
				...
				byte r = (byte)num;
				byte g = (byte)((this.num > 1) ? src[(res + z) * res + i] : 0);
				byte b = (byte)((this.num > 2) ? src[(2 * res + z) * res + i] : 0);
				byte a = (byte)((this.num > 3) ? src[(3 * res + z) * res + i] : 0);
				cols[z * res + i] = new Color32(r, g, b, a);
			}
		});
