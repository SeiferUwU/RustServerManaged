	...
	public void Think()
	{
		...
		float CalculatePreferredY(out bool reference)
		{
			...
			if (magnitude < 0.5f)
			{
				...
				reference = Physics.Raycast(origin, Vector3.down, out var hitInfo, 1000f, config.layerMask);
				if (!reference)
				{
					return num4;
				}
			...
			float a = GetMinimumHeight(Vector3.zero);
			float b = GetMinimumHeight(new Vector3(0f, 0f, num5 / 2f));
			float num6 = Mathf.Max(b: GetMinimumHeight(new Vector3(0f, 0f, num5)), a: Mathf.Max(a, b)) + preferredCruiseHeight;
			...
			reference = Physics.BoxCast(vector2, halfExtents, Vector3.down, out var hitInfo2, quaternion, 1000f, config.layerMask);
			if (reference)
			{
				Ray ray = new Ray(vector2, Vector3.down);
				Vector3 b2 = RayEx.ClosestPoint(ray, hitInfo2.point);
				float num7 = Vector3.Distance(ray.origin, b2);
