		...
		else
		{
			...
			foreach (SpawnInfoGroup spawnInfoGroup3 in array6)
			{
				...
				foreach (PathList rail in TerrainMeta.Path.Rails)
				{
					...
					{
						Vector3 vector3 = (rail.Spline ? path.GetPointCubicHermite(num11) : path.GetPoint(num11));
						Vector3 normalized3 = (path.GetPoint(num11 + num6) - path.GetPoint(num11 - num6)).normalized;
						for (int num12 = Mathf.RoundToInt(PositionOffset); num12 <= Mathf.CeilToInt((float)PositionOffset * 1.5f); num12 += 25)
						{
							for (int num13 = -1; num13 <= 1; num13 += 2)
							{
								Quaternion quaternion = Quaternion.LookRotation(num13 * normalized3.XZ3D());
								Vector3 vector4 = vector3;
								Quaternion quaternion2 = quaternion;
								Vector3 localScale = prefab3.Object.transform.localScale;
								quaternion2 *= Quaternion.LookRotation(normalized);
								vector4 -= quaternion2 * (zero + num12 * normalized2);
			...
			for (int num16 = 0; num16 < 8; num16++)
			{
				...
				foreach (SpawnInfoGroup spawnInfoGroup4 in array6)
				{
					...
					int num17 = (int)((!prefab4.Parameters) ? PrefabPriority.Low : (prefab4.Parameters.Priority + 1));
