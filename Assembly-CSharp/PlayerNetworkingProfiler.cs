...
public static class PlayerNetworkingProfiler
{
	...
	public static void Serialize(AzureAnalyticsUploader uploader, AnalyticsTable table, int frameIndex, DateTime timestamp)
	{
		...
		{
			SerializeRaknet(uploader, table, frameIndex, timestamp, server);
		}
	}

	private unsafe static void SerializeRaknet(AzureAnalyticsUploader uploader, AnalyticsTable table, int frameIndex, DateTime timestamp, Facepunch.Network.Raknet.Server server)
	{
