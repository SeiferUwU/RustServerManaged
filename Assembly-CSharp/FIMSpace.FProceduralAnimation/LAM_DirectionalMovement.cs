...
public class LAM_DirectionalMovement : LegsAnimatorControlModuleBase
{
	...
	private class LegRedirectHelper
	{
		...
		public Vector3 ComputeIKOffset(Vector3 localPos, float smoother = 1f)
		{
			...
			float num2 = Vector3.Magnitude(vector - computedPosLocal);
			...
			computedPosLocal = Vector3.Lerp(computedPosLocal, vector, LA.DeltaTime * (Mathf.Lerp(20f, 4f, (parent._calc_angleDiffFactor * num + num4) * 1.5f) + num5));
	...
	public override void OnLateUpdatePreApply(LegsAnimator.LegsAnimatorCustomModuleHelper helper)
	{
		...
		val3 *= num * _mainBlend * base.LA.ScaleReference;
		_calc_ikOff = val3;
		...
		val.y -= _calc_lStretch * User_StretchRotatorAnglePower * num * num14;
		val.y += _calc_rStretch * User_StretchRotatorAnglePower * num * num14;
		...
		if (num15 > 0f)
		{
			SpineBone.rotation = Quaternion.Slerp(SpineBone.rotation, b, Mathf.Lerp(1f, num15, _mainBlend));
		}
		_calc_hipsStretchOffset = Vector3.SmoothDamp(_calc_hipsStretchOffset, num6 * zero, ref _sd_hipsStretchOff, 0.2f + 0.3f * _play_TrDur.GetFloat(), 100000f, base.LA.DeltaTime);
		Vector3 vector = base.LA.RootToWorldSpaceVec(_calc_hipsPositionOffsets * 0.5f * base.LA.ScaleReference) * _mainBlend;
		base.LA._Hips_Modules_ExtraWOffset += vector + _calc_hipsStretchOffset * num4 * _mainBlend;
