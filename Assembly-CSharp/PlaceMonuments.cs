...
public class PlaceMonuments : ProceduralComponent
{
	...
	public override void Process(uint seed)
	{
		...
		else
		{
			...
			int num4 = Mathf.RoundToInt((float)TargetCount * TargetCountWorldSizeMultiplier.Evaluate(World.Size));
			for (int num5 = 0; num5 < 8; num5++)
			{
				...
				foreach (Prefab<MonumentInfo> prefab in array5)
				{
					...
					int num6 = (int)((!prefab.Parameters) ? PrefabPriority.Low : (prefab.Parameters.Priority + 1));
					...
					for (int num10 = 0; num10 < 10000; num10++)
					{
						...
						if (num12 <= num9 || !prefab.ApplyTerrainFilters(pos, rot, scale) || !prefab.ApplyTerrainAnchors(ref pos, rot, scale, Filter) || !component.CheckPlacement(pos, rot, scale))
						{
							continue;
						}
