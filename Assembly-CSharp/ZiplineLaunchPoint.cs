...
public class ZiplineLaunchPoint : BaseEntity
{
	...
	public void UpdateBuildingBlocks()
	{
		...
		{
			Vector3 vector3 = linePoints[num2];
			...
			float num3 = Mathf.Abs(vector5.y) + 2f;
			float z = Vector3.Distance(vector3, vector4);
			Vector3 size = (spawnBlocker.BoxCollider.size = new Vector3(0.5f, num3, z) + Vector3.one);
			longCollider.size = size;
			size = (spawnBlocker.BoxCollider.center = new Vector3(0f, 0f - num3 * 0.5f, 0f));
