		...
		for (int i = 0; i < DrawCalls.Count; i++)
		{
			...
			if (Render.IsMultidrawEnabled)
			{
				RenderParams rparams = new RenderParams
				{
					camera = camera,
					layer = 2097152,
					lightProbeUsage = drawCall.LightProbes,
					material = drawCall.MultidrawMaterial,
					worldBounds = cullingBounds,
					shadowCastingMode = drawCall.ShadowMode,
					receiveShadows = drawCall.ReceiveShadows,
					matProps = drawCall.MaterialBlock
				};
