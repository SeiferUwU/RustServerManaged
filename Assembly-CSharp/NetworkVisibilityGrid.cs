...
public class NetworkVisibilityGrid : MonoBehaviour, Provider
{
	...
	public Group GetGroup(Vector3 vPos)
	{
		...
		Group obj = Network.Net.sv.visibility.Get(iD);
		if (ConVar.Net.network_group_debug && !IsInside(obj, vPos))
		{
	...
	private void GetVisibleFrom(Group group, ListHashSet<Group> groups, int radius)
	{
		...
		var (num, num2, groupLayer) = DeconstructGroupId(iD);
		AddLayers(num, num2, groupLayer);
		for (int i = 1; i <= radius; i++)
		{
			AddLayers(num - i, num2, groupLayer);
			AddLayers(num + i, num2, groupLayer);
			AddLayers(num, num2 - i, groupLayer);
			AddLayers(num, num2 + i, groupLayer);
			for (int j = 1; j < i; j++)
			{
				AddLayers(num - i, num2 - j, groupLayer);
				AddLayers(num - i, num2 + j, groupLayer);
				AddLayers(num + i, num2 - j, groupLayer);
				AddLayers(num + i, num2 + j, groupLayer);
				AddLayers(num - j, num2 - i, groupLayer);
				AddLayers(num + j, num2 - i, groupLayer);
				AddLayers(num - j, num2 + i, groupLayer);
				AddLayers(num + j, num2 + i, groupLayer);
			}
			AddLayers(num - i, num2 - i, groupLayer);
			AddLayers(num - i, num2 + i, groupLayer);
			AddLayers(num + i, num2 - i, groupLayer);
			AddLayers(num + i, num2 + i, groupLayer);
		}
		void Add(int groupX, int groupY, int layer)
		{
			groups2.Add(Network.Net.sv.visibility.Get(CoordToID(groupX, groupY, layer)));
		}
		void AddLayers(int groupX, int groupY, int num3)
		{
			Add(groupX, groupY, num3);
			if (num3 == 0)
			{
				Add(groupX, groupY, 1);
			}
