	...
	protected override void RunJob()
	{
		...
		try
		{
			...
			foreach (ContainerInputOutput splitOutput in splitOutputs)
			{
				...
				foreach (ContainerInputOutput splitInput in splitInputs)
				{
					...
					for (int i = vector2i.x; i <= vector2i.y; i++)
					{
						...
						if (filterItems.Count > 0)
						{
							...
							{
								flag5 = FilterHasItem(slot, out filter);
							}
						...
						if (flag6 && filter.Item1.TargetItem != null && filter.Item1.MaxAmountInOutput > 0 && splitOutput.Storage.Container.GetTotalItemAmount(slot, range.x, range.y) >= filter.Item1.MaxAmountInOutput)
						{
							flag = true;
							continue;
						}
						int num4 = (int)((float)Mathf.Min(MaxStackSizePerMove, slot.info.stackable) / (float)count);
						if (flag6 && filter.Item1.MinAmountInInput > 0)
						{
							if (filter.Item1.TargetItem != null && FilterMatchItem(filter.Item1, slot))
							{
								int totalItemAmount = container.GetTotalItemAmount(slot, vector2i.x, vector2i.y);
								num4 = Mathf.Min(num4, totalItemAmount - filter.Item1.MinAmountInInput);
							}
							else if (filter.Item1.TargetCategory.HasValue)
							{
								num4 = Mathf.Min(num4, container.GetTotalCategoryAmount(filter.Item1.TargetCategory.Value, range.x, range.y) - filter.Item1.MinAmountInInput);
							}
							if (num4 == 0)
							{
								continue;
						...
						if (flag6 && filter.Item1.BufferAmount > 0)
						{
							num4 = Mathf.Min(num4, filter.Item1.BufferTransferRemaining);
						}
						if (flag6 && filter.Item1.MaxAmountInOutput > 0)
						{
							if (filter.Item1.TargetItem != null && FilterMatchItem(filter.Item1, slot))
							{
								num4 = Mathf.Min(num4, filter.Item1.MaxAmountInOutput - container2.GetTotalItemAmount(slot, range.x, range.y));
							}
							else if (filter.Item1.TargetCategory.HasValue)
							{
								num4 = Mathf.Min(num4, filter.Item1.MaxAmountInOutput - container2.GetTotalCategoryAmount(filter.Item1.TargetCategory.Value, range.x, range.y));
							}
							if ((float)num4 <= 0f)
							{
								flag = true;
						...
						num4 = (int)num5;
						if (num4 <= 0 || !container2.QuickIndustrialPreCheck(slot, range, 0, out var foundSlot))
						{
							continue;
						}
						...
						if (!flag7)
						{
							for (int j = range.x; j <= range.y; j++)
							{
								...
								{
									if ((item ?? slot).MoveToContainer(container2, j, allowStack: true, ignoreStackLimit: false, null, allowSwap: false))
									{
										flag7 = true;
						...
						if (filter.Item1.BufferTransferRemaining > 0)
						{
							var (value, _) = filter;
						...
						if (flag7)
						{
							...
							if (item != null)
							{
								AddTransfer(item.info.itemid, num6, splitInput.Storage.IndustrialEntity, splitOutput.Storage.IndustrialEntity);
							}
							else
							{
				...
				if (strictMode && !flag3 && transferStopWatch.Elapsed.TotalMilliseconds >= (double)(ConVar.Server.industrialFrameBudgetMs * 3f) && !isFirstTransfer)
				{
					multiFrameTransferInProcess = true;
					multiFrameOutputIndex = num;
					multiFrameInputIndex = 0;
			...
			{
				if (transferStopWatch.Elapsed.TotalMilliseconds >= (double)(ConVar.Server.industrialFrameBudgetMs * 3f))
				{
					strictMode = true;
		...
		void AddTransfer(int itemId, int amount, BaseEntity fromEntity, BaseEntity toEntity)
		{
			if (transfer != null && transfer.ItemTransfers != null)
			{
				...
				IndustrialConveyorTransfer.ItemTransfer item2 = new IndustrialConveyorTransfer.ItemTransfer
				{
					itemId = itemId,
					amount = amount
				};
		...
		static bool FilterMatchItem(ItemFilter itemFilter, Item item2)
		{
			if (itemFilter.TargetItem != null && (itemFilter.TargetItem == item2.info || (item2.IsBlueprint() == itemFilter.IsBlueprint && itemFilter.TargetItem == item2.blueprintTargetDef)))
			{
				return true;
			}
			return false;
