...
public class BaseMission : BaseScriptableObject
{
	...
	public class PositionGenerator
	{
		...
		public bool Validate(BasePlayer assignee, BaseMission missionDef)
		{
			...
			{
				List<MissionPoint> points = Facepunch.Pool.Get<List<MissionPoint>>();
	...
	public ItemAmount[] GetMissionRewards()
	{
		...
		if (alternativeRewards != null)
		{
			...
			foreach (MissionRewards missionRewards in array)
			{
				...
				{
					if (eras[j] == ConVar.Server.Era && missionRewards.items != null)
					{
	...
	public virtual void MissionComplete(MissionInstance instance, BasePlayer assignee)
	{
		...
		if (mission != null)
		{
			...
			if (missionRewards != null)
			{
				...
				foreach (ItemAmount reward in array)
				{
		...
		if (Rust.GameInfo.HasAchievements && mission != null && !mission.blockMissionStat)
		{
		...
		static void GiveReward(BasePlayer player, ItemAmount itemAmount)
		{
			...
			{
				Item item = (itemAmount.isBP ? ItemManager.Create(ItemManager.blueprintBaseDef, Mathf.CeilToInt(itemAmount.amount), 0uL) : ItemManager.Create(itemAmount.itemDef, Mathf.CeilToInt(itemAmount.amount), 0uL));
				if (itemAmount.isBP)
				{
					item.blueprintTarget = itemAmount.itemDef.itemid;
				}
	...
	public static bool AssignMission(BasePlayer assignee, IMissionProvider provider, BaseMission mission)
	{
		...
		for (int num2 = 0; num2 < mission.objectives.Length; num2++)
		{
			missionInstance.objectiveStatuses[num2] = new MissionInstance.ObjectiveStatus();
		}
