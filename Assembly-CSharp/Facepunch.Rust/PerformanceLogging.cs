...
public class PerformanceLogging
{
	...
	public void FlushMainThread()
	{
		...
		record.AddField("ram_managed", System.GC.GetTotalMemory(forceFullCollection: false)).AddField("ram_total", SystemInfoEx.systemMemoryUsed).AddField("total_session_id", totalSessionId.ToString("N"))
	...
	private Task ProcessPerformanceData(EventRecord record, List<TimeSpan> frametimes, List<TimeSpan> mainMenuFrametimes, List<TimeSpan> inventoryFrametimes, List<TimeSpan> craftingFrametimes, List<TimeSpan> contactsFrametimes, List<TimeSpan> mapFrametimes, List<long> gpuFrameTimes, List<int> ping)
	{
		...
		ApendFrameTimes(record, "", frametimes, extraTimings: true);
		if (isClient)
		{
		...
		return Task.CompletedTask;
		static void ApendFrameTimes(EventRecord eventRecord, string prefix, List<TimeSpan> times, bool extraTimings = false)
		{
			if (times.Count <= 1)
			{
				return;
			...
			double value2 = System.Math.Sqrt(pooledList.Sum((TimeSpan x) => System.Math.Pow(x.TotalMilliseconds - frametime_average, 2.0)) / (double)pooledList.Count - 1.0);
			...
			if (extraTimings)
			{
				eventRecord.AddLegacyTimespan(prefix + "frametime_99_9", pooledList[Mathf.Clamp(count - count / 1000, 0, count - 1)]).AddLegacyTimespan(prefix + "frametime_99", pooledList[Mathf.Clamp(count - count / 100, 0, count - 1)]);
			}
			eventRecord.AddLegacyTimespan(prefix + "frametime_90", pooledList[Mathf.Clamp(count - count / 10, 0, count - 1)]).AddLegacyTimespan(prefix + "frametime_75", pooledList[Mathf.Clamp(count - count / 4, 0, count - 1)]).AddLegacyTimespan(prefix + "frametime_50", pooledList[count / 2])
				.AddLegacyTimespan(prefix + "frametime_25", pooledList[count / 4])
				.AddLegacyTimespan(prefix + "frametime_10", pooledList[count / 10]);
			if (extraTimings)
			{
				eventRecord.AddLegacyTimespan(prefix + "frametime_1", pooledList[count / 100]).AddLegacyTimespan(prefix + "frametime_0_1", pooledList[count / 1000]);
			}
			eventRecord.AddField(prefix + "frametime_std_dev", value2);
		}
	}
