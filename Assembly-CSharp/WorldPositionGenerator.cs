...
public class WorldPositionGenerator : ScriptableObject
{
	...
	public bool TrySample(Vector3 origin, float minDist, float maxDist, out Vector3 position, List<Vector3> blocked = null)
	{
		...
		{
			ByteQuadtree.Element element = candidates[i];
			if (!element.IsLeaf)
			{
				candidates.RemoveUnordered(i--);
				EvaluateCandidate(element.Child1);
				EvaluateCandidate(element.Child2);
				EvaluateCandidate(element.Child3);
				EvaluateCandidate(element.Child4);
			}
		}
		...
		else
		{
			...
			while (true)
			{
				...
				ByteQuadtree.Element element2 = candidates[index];
				vector2 = GetElementRect(element2).center.XZ3D();
		...
		position = vector.WithY(aboveWater ? WaterLevel.GetWaterOrTerrainSurface(vector, waves: false, volumes: false) : TerrainMeta.HeightMap.GetHeight(vector));
		Pool.FreeUnmanaged(ref candidates);
		Pool.FreeUnmanaged(ref blockedRects);
		return true;
		void EvaluateCandidate(ByteQuadtree.Element child)
		{
			if (child.Value != 0)
			{
				Rect rect2 = GetElementRect(child);
				if (rect2.Overlaps(inclusion) && (!exclusion.Contains(rect2.min) || !exclusion.Contains(rect2.max)))
				{
					if (blockedRects.Count > 0)
					{
						foreach (Rect item3 in blockedRects)
						{
							if (item3.Contains(rect2.min) && item3.Contains(rect2.max))
							{
								return;
							}
						}
		...
		Rect GetElementRect(ByteQuadtree.Element element3)
		{
			int num2 = 1 << element3.Depth;
			float num3 = 1f / (float)num2;
			Vector2 vector3 = element3.Coords * num3;
