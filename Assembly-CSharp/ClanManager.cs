...
public class ClanManager : BaseEntity
{
	...
	public async void Server_GetLogs(RPCMessage msg)
	{
		...
		if (CheckClanResult(requestId, msg.player, result, out var clan))
		{
			...
			{
				ClientRPC(RpcTarget.Player("Client_ReceiveClanLogs", msg.player), ClanLogExtensions.ToProto(clanValueResult.Value));
			}
	...
	public async void Server_GetScoreEvents(RPCMessage msg)
	{
		...
		if (CheckClanResult(requestId, msg.player, result, out var clan))
		{
			...
			if (clanValueResult.IsSuccess)
			{
				ClientRPCPlayer(null, msg.player, "Client_ReceiveClanScoreEvents", ClanLogExtensions.ToProto(clanValueResult.Value));
			}
	...
	public async void Server_GetInvitations(RPCMessage msg)
	{
		...
		if (clanValueResult.IsSuccess)
		{
			ClientRPC(RpcTarget.Player("Client_ReceiveClanInvitations", msg.player), ClanInvitationExtensions.ToProto(clanValueResult.Value));
		}
	...
	public async void Server_GetLeaderboard(RPCMessage msg)
	{
		...
		{
			ClientRPCPlayer(null, msg.player, "Client_ReceiveClanLeaderboard", ClanLeaderboardExtensions.ToProto(_leaderboardCache));
		}
