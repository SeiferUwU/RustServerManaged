...
public class BaseSculpture : IOEntity, IServerFileReceiver, IUGCBrowserEntity, ISplashable, IDisposable
{
	...
	private void CarveCylinder(Vector3 worldSpacePosition, Vector3 worldSpaceView, int r, int depth)
	{
		...
		CarveAndBlurCylinderJob jobData = new CarveAndBlurCylinderJob
		{
			Grid = _grid,
			P0 = inBlockSpaceFloat,
			P1 = inBlockSpaceFloat + (float3)vector * (float)depth,
			R = r
		};
	...
	private void SmoothCylinder(Vector3 worldSpacePosition, Vector3 worldSpaceView, int r, int depth)
	{
		Vector3 vector = clientMeshCollider.transform.InverseTransformDirection(worldSpaceView);
		float3 inBlockSpaceFloat = GetInBlockSpaceFloat(worldSpacePosition);
		BoxBlurCylinderJob jobData = new BoxBlurCylinderJob
		{
			Grid = _grid,
			P0 = inBlockSpaceFloat,
			P1 = inBlockSpaceFloat + (float3)vector * (float)depth,
	...
	private void AddSphere(Vector3 worldSpacePosition, int r)
	{
		...
		global::AddAndBlurSphereJob jobData = new global::AddAndBlurSphereJob
		{
			Grid = _grid,
			Origin = inBlockSpace,
			R = r
	...
	{
		int3 inBlockSpace = GetInBlockSpace(worldSpacePosition);
		global::CarveAndBlurSphereJob jobData = new global::CarveAndBlurSphereJob
		{
			Grid = _grid,
			Origin = inBlockSpace,
			R = r
	...
	{
		int3 inBlockSpace = GetInBlockSpace(worldSpacePosition);
		global::BoxBlurSphereJob jobData = new global::BoxBlurSphereJob
		{
			Grid = _grid,
			Origin = inBlockSpace,
			R = r
	...
	public int DoSplash(ItemDefinition splashType, int amount)
	{
		...
		if (splashType == WaterTypes.WaterItemDef)
		{
			...
			global::BoxBlur3DJob jobData = new global::BoxBlur3DJob
			{
				InputGrid = _grid,
				OutputGrid = other,
				Width = _grid.Width,
	...
	{
		return new global::CleanFloatingIslandsJob
		{
			Sampler = _grid
		}.Schedule();
	}
