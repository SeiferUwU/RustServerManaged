...
public class FishShoal : IDisposable
{
	...
	private unsafe void UpdateJobs(float3 cameraPosition)
	{
		...
		if (num != 0)
		{
			...
			FishCollisionGatherJob jobData = new FishCollisionGatherJob
			{
				layerMask = -1,
				seed = seed,
				castCount = castCount,
			...
			FishCollisionProcessJob jobData2 = new FishCollisionProcessJob
			{
				castCount = castCount,
			...
			FishUpdateJob jobData3 = new FishUpdateJob
			{
				...
				fishDataArray = (FishData*)fishData.GetUnsafePtr(),
				fishRenderDataArray = (FishRenderData*)fishRenderData.GetUnsafePtr(),
				minDepth = item - 3f
			};
			KillFish jobData4 = new KillFish
			{
				fishCount = fishCount,
				fishDataArray = fishData,
				fishRenderDataArray = fishRenderData
