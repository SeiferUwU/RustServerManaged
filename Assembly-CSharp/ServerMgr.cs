...
public class ServerMgr : SingletonComponent<ServerMgr>, IServerCallback
{
	...
	private void Update()
	{
		...
		using (TimeWarning.New("ServerMgr.Update", 500))
		{
			...
			try
			{
				...
				{
					FSMComponent.workQueue.RunList(0.5);
				}
			}
	...
	private void UpdateServerInformation()
	{
		...
		using (TimeWarning.New("UpdateServerInformation"))
		{
			...
			SteamServer.GameTags = ServerTagCompressor.CompressTags($"mp{ConVar.Server.maxplayers},cp{BasePlayer.activePlayerList.Count},pt{Network.Net.sv.ProtocolId},qp{SingletonComponent<ServerMgr>.Instance.connectionQueue.Queued},$r{text8},v{2615}{text3}{text5},{text},{text2},cs{text6}{text7},ts{RelationshipManager.maxTeamSize}");
	...
	Network.ServerInfo IServerCallback.GetServerInfo()
	{
		return new Network.ServerInfo
		{
			NetworkVersion = 2615,
			Changeset = (BuildInfo.Current?.Scm?.ChangeId ?? "0"),
			Hostname = ConVar.Server.hostname,
			Seed = World.Seed,
			WorldSize = World.Size,
			Level = LevelManager.CurrentLevelName,
			LevelUrl = World.Url,
	...
	private void OnGiveUserInformation(Message packet)
	{
		...
		else if (packet.connection.protocol > 2615)
		{
			DebugEx.Log("Kicking " + packet.connection?.ToString() + " - their protocol is " + packet.connection.protocol + " not " + 2615);
