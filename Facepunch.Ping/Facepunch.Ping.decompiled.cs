...
public static class PingEstimater
{
	...
	private static async Task CalculateClosestRegion()
	{
		...
		for (int sampleCount = 0; sampleCount < numSamples; sampleCount++)
		{
			if (logging)
			{
				UnityEngine.Debug.Log($"[PingEstimator] Iteration {sampleCount}");
			}
			samples.Shuffle(ref seed);
			foreach (PingSample sample in samples)
			{
				sample.PingInstance = new UnityEngine.Ping(sample.Region.IP);
				...
				if (!parallel)
				{
					...
					if (logging)
					{
						UnityEngine.Debug.Log($"[PingEstimator] Region {sample.Region.Shortname} took {sample.PingInstance.time}ms");
					}
				}
			}
			if (parallel)
			...
			if (logging)
			{
				UnityEngine.Debug.Log($"[PingEstimator] Closest region is {pingSample.Region.Shortname} with {pingSample.Value}ms");
			}
		}
	}
}
